...
class TerminalUI:
 ...
 ╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝ ╚═╝  ╚═══╝ ╚═════╝ ╚══════╝
        ...
        subtitle = "─── KRONOS CALCULATOR ULTRA v3.0 by Kronos English ───"
    ...
    def get_input(
        self, prompt: str, type_func=str, validator=None, error_msg="Invalid input."
    ):
        while True:
            try:
                val = input(prompt)
...
class Vector3D:
    ...
    def from_list(cls, lst: list) -> "Vector3D":
        if len(lst) != 3:
            raise ValueError("Vector3D must consist of exactly 3 elements [x, y, z]")
...
class MatrixEngine:
        ...
        if matrix_str.startswith("[") and matrix_str.endswith("]"):
            try:
                ...
                if not matrix or any(len(row) != len(matrix[0]) for row in matrix):
                    raise ValueError("Rows of the matrix have different lengths or the matrix is empty")
                return matrix
            except Exception as e:
            ...
            try:
                ...
                raise ValueError(f"Error parsing simplified matrix: {e}")
    ...
    def multiply(A: List[List[complex]], B: List[List[complex]]) -> List[List[complex]]:
        ...
        if cols_A != rows_B:
            raise ValueError(f"Incompatible matrix dimensions: {cols_A} != {rows_B}")
    ...
    def solve_linear_system(
        ...
        if any(len(row) != n for row in A):
            raise ValueError("Matrix of coefficients A must be square")
        if len(B) != n:
            raise ValueError("Height of B must be equal to the height of A")
        m_B = len(B[0])
...
class UnitConverter:
    ...
    def convert_temperature(cls, value: float, from_unit: str, to_unit: str) -> float:
        ...
        if from_u == "c":
            ...
            raise ValueError("Unknown temperature unit. Use: c, k, f")
...
class KronosApp:
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            # --- ОТРИСОВКА МЕНЮ ---
            print_header("ALGEBRA AND CALCULUS")
            print_item(1, "Calculator (Basic/Scientific)")
            print_item(2, "Vectors (Dot/Cross product)")
            print_item(3, "Linear Algebra (Matrices)")
            print_item(4, "Algebraic Equations")
            print_item(5, "Calculus (Derivatives/Integrals)")
            print_item(6, "Geometry (Triangles)")
            print_item(7, "Number Theory (GCD, LCM, Primes)")

            print_header("ENGINEERING AND ANALYTICS")
            print_item(8, "Physical Units Converter")
            print_item(9, "Physics (Kinematics Solver)")
            print_item(10, "Chemistry (Mass Fractions, Molar Mass)")
            print_item(11, "Statistical Analysis")
            print_item(12, "Plotting Graphs (Matplotlib / ASCII)")

            print_header("TOOLS")
            print_item(13, "Cryptography (Hashing)")
            print_item(14, "Number Systems")
            print_item(15, "Calculation Log (Asynchronous)")
            print_item(16, "Run Tests")
            ...
            # --- ЛОГИКА ВВОДА ---
            prompt_str = f"\033[1m{rgb(*head_text, '» Choose an option: ')}\033[0m"
            error_str = rgb(*exit_color, "Invalid input.")
            ...
            if choice == 0:
                self.ui.print_smart(
                    f"\n\033[1m{rgb(255, 0, 128, 'Session closed. See you later!')}\033[0m"
                )
                break
            ...
            try:
                ...
                error_msg = f"\n{rgb(*exit_color, 'Critical error in module: ')}{e}"
                self.ui.print_smart(error_msg)
            
            self.ui.pause()

    def mod_converter(self) -> None:
        """Interactive engineering unit converter (Item 8)"""
        while True:
            self.ui.clear_screen()
            print(
                f"   {Colors.BOLD}{Colors.YELLOW}───[ PHYSICAL UNITS CONVERTER ]───{Colors.RESET}\n"
            )
            print(
                f"   {Colors.CYAN}[1]{Colors.RESET} Length (m, mm, cm, km, inch, foot, yard, mile)"
            )
            print(
                f"   {Colors.CYAN}[2]{Colors.RESET} Mass (kg, g, mg, ton, pound, ounce)"
            )
            print(
                f"   {Colors.CYAN}[3]{Colors.RESET} Pressure (pa, kpa, mpa, bar, atm, psi)"
            )
            print(f"   {Colors.CYAN}[4]{Colors.RESET} Temperature (c, k, f)")
            ...
            choice = self.ui.get_input(
                f"{Colors.BOLD}{Colors.CYAN}» Choose a category: {Colors.RESET}",
                int,
                lambda x: 0 <= x <= 4,
                "Invalid choice.",
            )
            ...
            try:
                if choice == 1:
                    print(
                        f"{Colors.BOLD}{Colors.YELLOW}Available length units:{Colors.RESET} {', '.join(UnitConverter.LENGTH_FACTORS.keys())}"
                    )
                    from_unit = input(f"From which unit are we converting? ").strip().lower()
                    to_unit = input(f"To which unit are we converting? ").strip().lower()
                    value = self.ui.get_input(
                        "Enter the value: ", float, error_msg="Invalid number."
                    )
                    ...
                    print(
                        f"{Colors.BOLD}{Colors.YELLOW}Available mass units:{Colors.RESET} {', '.join(UnitConverter.MASS_FACTORS.keys())}"
                    )
                    from_unit = input(f"From which unit are we converting? ").strip().lower()
                    to_unit = input(f"To which unit are we converting? ").strip().lower()
                    value = self.ui.get_input(
                    ...
                    print(
                        f"{Colors.BOLD}{Colors.YELLOW}Available pressure units:{Colors.RESET} {', '.join(UnitConverter.PRESSURE_FACTORS.keys())}"
                    )
                    from_unit = input(f"From which unit are we converting? ").strip().lower()
                    to_unit = input(f"To which unit are we converting? ").strip().lower()
                    value = self.ui.get_input(
                    ...
                    print(
                        f"{Colors.BOLD}{Colors.YELLOW}Available temperature units:{Colors.RESET} c (Celsius), k (Kelvin), f (Fahrenheit)"
                    )
                    from_unit = (
                        input(f"From which unit are we converting (c/k/f)? ").strip().lower()
                    )
                    to_unit = (
                        input(f"To which unit are we converting (c/k/f)? ").strip().lower()
                    )
                    value = self.ui.get_input(
                        "Enter the value: ", float, error_msg="Invalid number."
                    )
    ...
    def mod_calculator(self) -> None:
        self.ui.clear_screen()
        print(
            f"   {Colors.BOLD}{Colors.YELLOW}───[ ADVANCED CALCULATOR ]───{Colors.RESET}\n"
        )
        print(
            f"Supported: complex numbers ({Colors.GREEN}2+3j{Colors.RESET}), parentheses, functions ({Colors.GREEN}sin, cos, sqrt{Colors.RESET})"
        )
        print(
            f"Constants from the database: {Colors.CYAN}pi, e, c{Colors.RESET} (speed of light), {Colors.CYAN}G{Colors.RESET} (gravitational) and others\n"
        )

        expr = input(
            f"{Colors.BOLD}{Colors.CYAN}» Enter expression: {Colors.RESET}"
        ).strip()
        if not expr:
            return
        ...
        try:
            ...
            0
            print(
                f"\n  {Colors.BOLD}{Colors.GREEN}[RESULT]{Colors.RESET} {Colors.BRIGHT_CYAN}{res_str}{Colors.RESET}"
            )

            self.history.add("Calculator", expr, res_str)

        except Exception as e:
            self.ui.print_smart(f"\n{Colors.RED}[CALCULATION ERROR] {e}{Colors.RESET}")

    def mod_vectors(self) -> None:
        self.ui.print_smart(
            f"{Colors.BOLD}{Colors.CYAN}--- VECTOR ALGEBRA ---{Colors.RESET}"
        )
        print(
            f"[{Colors.CYAN}1{Colors.RESET}] Scalar Product (Dot)\n[{Colors.CYAN}2{Colors.RESET}] Vector Product (Cross)"
        )
        sub = self.ui.get_input(
            f"{Colors.YELLOW}Choice: {Colors.RESET}", int, lambda x: x in [1, 2]
        )

        def get_vec(name: str) -> Vector3D:
            s = self.ui.get_input(f"Vector {name} (X Y Z separated by spaces): ", str)
        ...
        if sub == 1:
            ...
            if mag1 > 0 and mag2 > 0:
                ...
                self.ui.print_smart(
                    f"{Colors.GREEN}Angle between vectors: {angle:.2f}°{Colors.RESET}"
                )
        else:
            res_v = v1.cross(v2)
            self.ui.print_smart(
    ...
    def mod_converter(self) -> None:
        self.ui.print_smart(
            f"{Colors.BOLD}{Colors.CYAN}--- UNIT CONVERTER ---{Colors.RESET}"
        )
        print(
            f"[{Colors.CYAN}1{Colors.RESET}] Length {list(UnitConverter.LENGTH.keys())}"
        )
        print(f"[{Colors.CYAN}2{Colors.RESET}] Mass {list(UnitConverter.MASS.keys())}")
        print(
            f"[{Colors.CYAN}3{Colors.RESET}] Pressure {list(UnitConverter.PRESSURE.keys())}"
        )
        print(
            f"[{Colors.CYAN}4{Colors.RESET}] Energy {list(UnitConverter.ENERGY.keys())}"
        )
        print(f"[{Colors.CYAN}5{Colors.RESET}] Temperature ['C', 'K', 'F']")
        ...
        c = self.ui.get_input(
            f"{Colors.YELLOW}Category (1-5): {Colors.RESET}",
            int,
            lambda x: 1 <= x <= 5,
        )

        val = self.ui.get_input("Value: ", float)
        u1 = self.ui.get_input("From (e.g., kg): ", str)
        u2 = self.ui.get_input("To (e.g., lb): ", str)
        ...
        try:
            ...
            self.history.add("Unit Converter", f"{val} {u1} -> {u2}", f"{res:.6g}")
        except Exception as e:
            self.ui.print_smart(f"{Colors.RED}Error: {e}{Colors.RESET}")

    def mod_plotting(self) -> None:
        self.ui.print_smart(f"{Colors.BOLD}{Colors.CYAN}--- PLOTS ---{Colors.RESET}")
        print(
            f"[{Colors.CYAN}1{Colors.RESET}] Matplotlib (Cartesian)\n[{Colors.CYAN}2{Colors.RESET}] ASCII (In console - Symbolic render)"
        )
        mode = self.ui.get_input(
            f"{Colors.YELLOW}Choice: {Colors.RESET}", int, lambda x: x in [1, 2]
        )
        func_str = self.ui.get_input("Function f(x): ", str)
        ...
        if mode == 1:
            try:
                ...
                self.ui.print_smart(
                    f"{Colors.RED}Error rendering (install matplotlib/numpy): {e}{Colors.RESET}"
                )
        else:
            self.ui.print_smart(
                f"{Colors.MAGENTA}Rendering ASCII plot...{Colors.RESET}\n"
            )
            w, h = 70, 20
            x_min, x_max = -10, 10
    ...
    def mod_matrix(self) -> None:
        ...
        try:
            ...
            print(f"\n{Colors.GREEN}Matrix A ({len(A)}x{len(A[0])}):{Colors.RESET}")
            print(MatrixEngine.to_string(A))

            print(f"\n[{Colors.CYAN}1{Colors.RESET}] Find determinant (Det A)")
            print(f"[{Colors.CYAN}2{Colors.RESET}] Transpose (A^T)")
            print(f"[{Colors.CYAN}3{Colors.RESET}] Find inverse matrix (A^-1)")
            print(f"[{Colors.CYAN}4{Colors.RESET}] Multiply by second matrix (A * B)")
            print(
                f"[{Colors.CYAN}5{Colors.RESET}] Solve system of equations (A * X = B)"
            )

            act = self.ui.get_input(
                f"\n{Colors.YELLOW}Choose operation: {Colors.RESET}",
                int,
            ...
            if act == 1:
                ...
                raw_b = self.ui.get_input(
                    f"{Colors.CYAN}Enter matrix B: {Colors.RESET}", str
                )
                ...
                raw_b = self.ui.get_input(
                    f"{Colors.CYAN}Enter vector/matrix B: {Colors.RESET}", str
                )
                B = MatrixEngine.parse_matrix(raw_b)
                X = MatrixEngine.solve_linear_system(A, B)
                print(
                    f"\n{Colors.BOLD}{Colors.GREEN}[Solution X]:{Colors.RESET}\n{MatrixEngine.to_string(X)}"
                )
    ...
    def mod_equations(self) -> None:
        self.ui.print_smart(
            f"{Colors.BOLD}{Colors.CYAN}--- EQUATIONS ---{Colors.RESET}"
        )
        a, b, c, d = [
            self.ui.get_input(f"Coefficient {k}: ", float) for k in ("a", "b", "c", "d")
        ]
    ...
    def mod_calculus(self) -> None:
        self.ui.print_smart(
            f"{Colors.BOLD}{Colors.CYAN}--- CALCULUS ---{Colors.RESET}"
        )
        expr = self.ui.get_input("Function f(x): ", str)
        x_val = self.ui.get_input("Point x for derivative: ", float)
        h = 1e-6
    ...
    def mod_geometry(self) -> None:
        self.ui.print_smart(
            f"{Colors.BOLD}{Colors.CYAN}--- GEOMETRY (TRIANGLES) ---{Colors.RESET}"
        )
        a, b, c = [
            self.ui.get_input(f"Side {k}: ", float, lambda x: x > 0)
            for k in ("a", "b", "c")
        ]
        if a + b <= c or a + c <= b or b + c <= a:
            self.ui.print_smart(
                f"{Colors.RED}Triangle with such sides does not exist!{Colors.RESET}"
            )
            return
        p = (a + b + c) / 2
        S = math.sqrt(p * (p - a) * (p - b) * (p - c))
        print(f"\n  {Colors.YELLOW}Area (S):{Colors.RESET} {S:.4g}")
        print(
            f"  {Colors.YELLOW}Circumradius (R):{Colors.RESET} {(a * b * c) / (4 * S):.4g}"
        )

    def mod_number_theory(self) -> None:
        self.ui.print_smart(
            f"{Colors.BOLD}{Colors.CYAN}--- NUMBER THEORY ---{Colors.RESET}"
        )
        a = self.ui.get_input("Number A: ", int)
        b = self.ui.get_input("Number B: ", int)
        self.ui.print_smart(f"\n  {Colors.GREEN}GCD:{Colors.RESET} {math.gcd(a, b)}")
        self.ui.print_smart(f"  {Colors.GREEN}LCM:{Colors.RESET} {math.lcm(a, b)}")

    def mod_physics(self) -> None:
        self.ui.print_smart(
            f"{Colors.BOLD}{Colors.CYAN}--- PHYSICS (Kinematic Solver) ---{Colors.RESET}"
        )
        self.ui.print_smart(
            f"{Colors.GRAY}Enter 3 known parameters. Leave unknowns empty.{Colors.RESET}"
        )
        knowns = {
    ...
    def mod_chemistry(self) -> None:
        ...
        formula = self.ui.get_input("Formula (e.g., C2H5OH): ", str)
    ...
    def mod_statistics(self) -> None:
        self.ui.print_smart(
            f"{Colors.BOLD}{Colors.CYAN}--- STATISTICS ---{Colors.RESET}"
        )
        raw = self.ui.get_input("Sample (numbers separated by spaces): ", str)
    ...
    def mod_cryptography(self) -> None:
        text = self.ui.get_input("String: ", str)
        print(
    ...
    def mod_history_center(self) -> None:
        for item in self.history.items[-20:]:
            print(item.to_line())
        act = self.ui.get_input(
            f"\n[{Colors.CYAN}1{Colors.RESET}] Export TXT [{Colors.CYAN}2{Colors.RESET}] Export CSV [{Colors.CYAN}3{Colors.RESET}] Clear: ",
            int,
            lambda x: x in [1, 2, 3],
        )
        if act == 1:
            self.ui.print_smart(
                f"{Colors.GREEN}Saved: {self.history.export_txt()}{Colors.RESET}"
            )
        elif act == 2:
            self.ui.print_smart(
                f"{Colors.GREEN}Saved: {self.history.export_csv()}{Colors.RESET}"
            )
        elif act == 3:
            self.history.clear()
            self.ui.print_smart(f"{Colors.RED}Cleared.{Colors.RESET}")
